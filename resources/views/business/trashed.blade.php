<x-app-layout>
    ...
    <div class="w-80pc h-50pc mx-auto">
        <div class="w-full h-50pc">
            ...
            <form method="GET" action="{{ route('businesses.trashed') }}">
                <div class="min-w-200px flex items-end justify-left overflow-hidden flex-warp flex-gap-10px">
                    ...
                    {{-- Clear --}}
                    <a class="btn btn-secondary" href="{{ route('businesses.trashed') }}">{{ __('btn.clear_filter') }}</a>
                </div>
            </form>
            <br>
                                ...
                                <tr>
                                    ...
                                    <td class="flex justify-center flex-gap-3px">
                                        <form method="POST" action="{{ route('businesses.restore', $business->id) }}">
                                            @csrf
                                            @method('post')
                                            <button class="btn btn-success color-success" type="submit"
                                                onclick="confirm(trans('message.confirm', {'action': trans('btn.restore')}))">{{ __('btn.restore') }}</button>
                                        </form>
