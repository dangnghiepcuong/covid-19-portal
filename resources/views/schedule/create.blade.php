<x-app-layout>
    ...
    <div class="flex justify-center">
        <form method="POST" action="{{ route('schedules.store') }}" class="w-full max-w-50pc">
            @csrf
            ...
            <div class="mt-4">
                ...
                <x-input id="day_shift_limit" class="block mt-1 w-full" type="number" name="day_shift_limit" :value="old('day_shift_limit') !== null ? old('day_shift_limit') : 0" />
            </div>

            <!-- noon_shift_limit -->
            <div class="mt-4">
                <x-label for="noon_shift_limit" :value="__('schedule.noon_shift_limit')" />
                <x-input id="noon_shift_limit" class="block mt-1 w-full" type="number" name="noon_shift_limit" :value="old('noon_shift_limit') !== null ? old('noon_shift_limit') : 0" />
            </div>

            <!-- night_shift_limit -->
            <div class="mt-4">
                <x-label for="night_shift_limit" :value="__('schedule.night_shift_limit')" />
                <x-input id="night_shift_limit" class="block mt-1 w-full" type="number" name="night_shift_limit" :value="old('night_shift_limit') !== null ? old('night_shift_limit') : 0" />
            </div>
            ...
            @if (Session::get('success'))
                <div class="alert alert-success">
                    <ul>
                        <li>{{ __('message.success', ['action' => __('btn.create', ['object' => __('schedule.schedule')])]) }}</li>
                    </ul>
                </div>
            @endif
        </form>
